...
or union as 1. E.g.,
for `struct { struct {} e[1]; float f; }` as the first argument, 
C will treat it like `struct { float f; }` and pass `f` in fa0 as described below, 
whereas {Cpp} will pass the pass the entire aggregate in a0 (XLEN = 64) 
or a0 and a1 (XLEN = 32), as described in the integer calling convention.
